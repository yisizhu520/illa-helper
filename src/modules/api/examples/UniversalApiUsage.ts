...
export class UniversalApiUsageExamples {
  ...
  static async textAnalysis() {
    const result = await callAI(
      ...
      {
        systemPrompt: '你是一个专业的文本情感分析师，请对用户提供的文本进行情感分析。',
        temperature: 0.3
      }
    );
  ...
  static async chatConversation() {
    const messages = [
      { role: 'system' as const, content: '你是一个友善的AI助手，专门帮助用户解决技术问题。' },
      { role: 'user' as const, content: '我在学习React，但是对useState钩子不太理解' },
      { role: 'assistant' as const, content: 'useState是React中最基础的Hook之一，它让函数组件能够拥有状态...' },
  ...
  static async seoTitleGeneration() {
    const result = await universalApi.call(
      ...
      {
        systemPrompt: '你是一名SEO专家，擅长创建吸引点击的标题。标题要包含关键词，长度控制在60字符以内。',
        temperature: 0.8
      }
    );
