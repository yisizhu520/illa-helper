    ...
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="space-y-2">
            ...
            <p class="text-muted-foreground">
                管理不需要翻译功能的网站。支持使用通配符模式，如 <code
                    class="px-1.5 py-0.5 rounded bg-muted text-muted-foreground text-sm">*://github.com/*</code>
            </p>
        </div>
                ...
                <table class="w-full">
                    <thead class="bg-muted/50">
                        <tr>
                            ...
                            <th class="text-left p-4 font-medium text-foreground">添加时间</th>
                    ...
                    <tbody>
                        <tr v-for="(pattern, index) in filteredPatterns" :key="pattern.url"
                            ...
                            <td class="p-4">
                                <code class="px-2 py-1 bg-muted rounded text-sm font-mono">{{ pattern.url }}</code>
                            </td>
                            <td class="p-4">
                                <span
        ...
        <div class="text-sm text-muted-foreground">
            ...
            <span v-if="searchQuery">，显示 {{ filteredPatterns.length }} 个匹配结果</span>
        </div>
...
import { Search, Plus, Trash2, Download, Upload, Shield, Edit3 } from 'lucide-vue-next';
        ...
        if (file) {
            try {
                ...
                if (Array.isArray(importedData)) {
                    // 旧格式：直接是字符串数组
                    patternsToImport = importedData.filter(item => typeof item === 'string');
                } else if (importedData.patterns && Array.isArray(importedData.patterns)) {
                    // 新格式：包含完整数据的对象
                    patternsToImport = importedData.patterns.map((p: any) => p.url || p).filter(Boolean);
                } else {
                ...
                const newPatterns = patternsToImport.filter(pattern => !existingPatterns.includes(pattern));
